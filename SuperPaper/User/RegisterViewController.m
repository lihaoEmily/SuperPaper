...
- (IBAction)userRegister:(id)sender {
    if ([self checkInput]) {
        NSLog(@"准备注册！");
        __block NSString *mobile = self.telNumTextField.text;
        NSString *pwd = _pwd;
        NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@regist.php",BASE_URL]];
        NSString *params = [[NSString stringWithFormat:@"mobile=%@&password=%@",mobile,pwd]stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
        [request setHTTPMethod:@"POST"];
        [request setHTTPBody:[params dataUsingEncoding:NSUTF8StringEncoding]];
        ...
        NSURLSessionDataTask *task = [[NSURLSession sharedSession]dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            if (data) {
                NSError *newError;
                NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:data options:0 error:&newError];
                NSLog(@"%@,%@",dic,dic[@"message"]);
                if (!newError) {//json解析正确
