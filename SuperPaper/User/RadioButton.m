...
#define Unselected  @"RadioButton-Unselected"
#define Selected  @"RadioButton-Selected"
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        for(int i = 0;i < [options count] / columns;i++){
            for(int j = 0;j < columns; j++){
                ...
                [btTemp setImage:[UIImage imageNamed:Unselected] forState:UIControlStateNormal];
        ...
        for(int j = 0;j < rem;j++){
            ...
            int x = framex * 0.25;
            int y = framey * 0.25;
            UIButton *btTemp = [[UIButton   alloc] initWithFrame:CGRectMake(framex * j + x,
                                                         framey * ([options count] / columns),
                                                         framex / 2 + x,framey / 2 + y)];
            [btTemp addTarget:self action:@selector(radioButtonClicked:)
             forControlEvents:UIControlEventTouchUpInside];
            btTemp.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
            [btTemp setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
            [btTemp setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            btTemp.titleLabel.font =[UIFont systemFontOfSize:14.f];
            [btTemp setTitle:[options objectAtIndex:k] forState:UIControlStateNormal];
            [self.radioButtons addObject:btTemp];
...
-(void) radioButtonClicked:(UIButton *) sender{
    ...
    for(int i = 0;i < [self.radioButtons count];i++){
        [[self.radioButtons objectAtIndex:i] setImage:[UIImage
                                                       imageNamed:Unselected]
                                             forState:UIControlStateNormal];
    }
    [sender setImage:[UIImage imageNamed:Selected]
            forState:UIControlStateNormal];
...
-(void) setSelected:(int) index{
    for(int i = 0;i < [self.radioButtons count];i++){
        [[self.radioButtons objectAtIndex:i] setImage:[UIImage
                                                       imageNamed:Unselected] forState:UIControlStateNormal];
        
    }
    [[self.radioButtons objectAtIndex:index] setImage:[UIImage
                                                       imageNamed:Selected] forState:UIControlStateNormal];
